...
describe("profileInformationSchema", () => {
  ...
  describe("website_url", () => {
    it("rejects an invalid URL", () => {
      const result = profileInformationSchema.safeParse({ website_url: "not-a-url" });
      expect(result.success).toBe(false);
    });

    it("accepts a valid HTTPS URL", () => {
      const result = profileInformationSchema.safeParse({ website_url: "https://portfolio.dev" });
      expect(result.success).toBe(true);
    });

    it("accepts an empty string", () => {
      expect(profileInformationSchema.safeParse({ website_url: "" }).success).toBe(true);
    });
  });

  describe("job_role", () => {
    it("accepts a valid enum value", () => {
      expect(profileInformationSchema.safeParse({ job_role: "Frontend Engineer" }).success).toBe(true);
    });

    it("rejects an unlisted job role", () => {
      const result = profileInformationSchema.safeParse({ job_role: "Ninja Developer" });
      expect(result.success).toBe(false);
    });

    it("accepts an empty string", () => {
      expect(profileInformationSchema.safeParse({ job_role: "" }).success).toBe(true);
    });
  });

  describe("short_bio", () => {
    it("accepts exactly 250 characters", () => {
      expect(profileInformationSchema.safeParse({ short_bio: "a".repeat(250) }).success).toBe(true);
    });

    it("rejects 251 characters", () => {
      const result = profileInformationSchema.safeParse({ short_bio: "a".repeat(251) });
      expect(result.success).toBe(false);
  ...
  describe("x_username", () => {
    it("accepts exactly 50 characters", () => {
      expect(profileInformationSchema.safeParse({ x_username: "a".repeat(50) }).success).toBe(true);
    });

    it("rejects 51 characters", () => {
      const result = profileInformationSchema.safeParse({ x_username: "a".repeat(51) });
      expect(result.success).toBe(false);
  ...
  describe("location", () => {
    it("accepts exactly 100 characters", () => {
      expect(profileInformationSchema.safeParse({ location: "a".repeat(100) }).success).toBe(true);
    });

    it("rejects 101 characters", () => {
      const result = profileInformationSchema.safeParse({ location: "a".repeat(101) });
      expect(result.success).toBe(false);
  ...
  describe("review_description", () => {
    it("accepts exactly 200 characters", () => {
      expect(profileInformationSchema.safeParse({ review_description: "a".repeat(200) }).success).toBe(true);
    });

    it("rejects 201 characters", () => {
      const result = profileInformationSchema.safeParse({ review_description: "a".repeat(201) });
      expect(result.success).toBe(false);
  ...
  describe("review_stars", () => {
    it("accepts 0", () => {
      expect(profileInformationSchema.safeParse({ review_stars: 0 }).success).toBe(true);
    });

    it("accepts 5", () => {
      expect(profileInformationSchema.safeParse({ review_stars: 5 }).success).toBe(true);
    });
...
describe("walletAddressSchema", () => {
  ...
  it("passes with empty address (disconnect — no sig/message needed)", () => {
    expect(walletAddressSchema.safeParse({ wallet_address: "" }).success).toBe(true);
  });
  ...
  it("fails when address is set but signature is missing", () => {
    ...
    expect(result.success).toBe(false);
    expect(result.error?.errors[0].message).toContain("Signature and message are required");
  });
  ...
  it("fails with invalid base58 address", () => {
    ...
    expect(result.success).toBe(false);
    expect(result.error?.errors[0].message).toContain("Invalid Solana wallet address");
  });
