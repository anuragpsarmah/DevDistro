    ...
    if (store) {
        for (const [key, value] of Object.entries(context)) {
            if (value !== undefined && value !== null) {
                if (typeof value === "object" && !Array.isArray(value) && store[key] && typeof store[key] === "object") {
                    store[key] = { ...store[key] as Record<string, unknown>, ...value as Record<string, unknown> };
                } else {
                    store[key] = value;
                }
            }
